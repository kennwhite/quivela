...
function invariant4(ctx1: Context, ctx2: Context, addr1: Addr, addr2: Addr): bool
{
  ...
  var ret := Eval(ECVar(EVar("cpa"), "d", EConst(Nil())), ctx, FUEL).0;
  !ret.Error?
}
    ...
    Equivalent_AllMethods(ctx1, ctx2, addr1, addr2, invariant8)
{
    ...
    assert NoExternalEffects(newField.val) by {
      EConst_NoExternalEffects(Int(0));
    }
    assert UnusedIn(newField.name, lhs.body) by {
      assume false;
    }
    assert lhs.body == rhs.body;
    EquivalentAddNewField(prefix, lhs.locals, lhs.body, newField, invariant8, 3);
}
